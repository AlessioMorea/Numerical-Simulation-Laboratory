...
Genetic ::Genetic(string method, int rank, int size)
{
  Set(method, rank, size);
  int seed[4];
  int p1, p2;
  ifstream Primes("Primes");
  if (Primes.is_open())
  {
    for (int i = 0; i <= 3 * m_rank; i++)
    {
      Primes >> p1 >> p2;
    }
  }
  ...
  if (m_rank == 0)
  {
    if (method == "America")
    {
      ifstream CityAmerica;
      CityAmerica.open(method + "/American_capitals.in");
      double x, y;
  ...
  {
    Positions_x.resize(m_NumberCities);
    Positions_y.resize(m_NumberCities);
  }

  MPI_Bcast(Positions_x.data(), Positions_x.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Bcast(Positions_y.data(), Positions_y.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  {
    random_shuffle(begin(CityIndex) + 1, end(CityIndex), [=](int n)
                   { return int(rnd.Rannyu(0, n)); });
...
void Genetic::Print()
{
  ...
  m_Generation++;
  OutputBest.open(m_method + "/" + to_string(m_rank) + "output_best.dat", ios::app);
  ...
  OutputBestHalf.open(m_method + "/" + to_string(m_rank) + "HalfAverage.dat", ios::app);
...
void Genetic::Sort()
{
  ...
  for (int i = 0; i < m_NumberParents; i++)
  {
    ...
    sort(Chromosomes.begin() ,
    Chromosomes.end(), [=](int a, int b) {
    return FitnessCalculation(a)<FitnessCalculation(b);
    });
    for(int i=0;i<m_NumberParents;i++){
    cout<<i<<" "<<FitnessCalculation(i)<<endl;
    }*/
  }
}
...
void Genetic::Set(string method, int rank, int size)
{
  ifstream Input;
  m_rank = rank;
  m_size = size;
...
{
  ofstream Best;
  Best.open(m_method + "/" + to_string(m_rank) + "BestChr.dat", ios::app);
  for (int i = 0; i < m_NumberCities; i++)
  {
...
void Genetic::CompleteSim()
{
  ...
  for (int it = 1; it < m_N_Generation + 1; it++)
  {
    ...
    if (it == n_migr)
    {
      ...
      vector<int> irecv(m_size * m_NumberCities);
      vector<vector<int>> cp(m_size, vector<int>(m_NumberCities));

      MPI_Gather(Chromosomes[m_NumberParents - 1].data(), Chromosomes[m_NumberParents - 1].size(), MPI_INT, irecv.data(), Chromosomes[m_NumberParents - 1].size(), MPI_INT, 0, MPI_COMM_WORLD);
      ...
      if (m_rank == 0)
      {
        for (int i = 0; i < m_size; i++)
        {
          for (int j = 0; j < m_NumberCities; j++)
          {
            cp[i][j] = irecv[j + i * m_NumberCities];
          }
        }
        srand(it);
        ...
        for (int i = 0; i < m_size; i++)
        {
          for (int j = 0; j < m_NumberCities; j++)
          {
            irecv[j + i * m_NumberCities] = cp[i][j];
          }
        }
      }
      MPI_Scatter(irecv.data(), Chromosomes[m_NumberParents - 1].size(), MPI_INT, Chromosomes[m_NumberParents - 1].data(), Chromosomes[m_NumberParents - 1].size(), MPI_INT, 0, MPI_COMM_WORLD);
      n_migr += 20;
    }
    ...
    if (m_rank == 0)
    {
      ...
      {
        progressbar += "=";
       cout << progressbar << " > " << (double)it * 100 / m_N_Generation << " % "   << endl;
      }
    }
  }
}
