...
ofstream output(folder+"InstantValues.dat",ios::app);
output<<walker[iv]/(double)npart + vtail<<endl;
output.close();
...
//Print initial values for measured properties
  ...
  cout << "Initial pressure     (with Tail Corrections)    = " << rho*temp+(walker[ipr] + (double)npart * ptail) / vol << endl;
...
{
    double r, gdir;
    ofstream Gofr, Gave, Epot, Press;//Temp;
    ...
    Press.open(folder+"output_press.dat",ios::app);
    Gofr.open(folder+"output_gofr.dat",ios::app);
    Gave.open(folder+"output_gave.dat",ios::app);
    ...
    stima_press = rho * temp + (blk_av[ipr]/blk_norm + ptail * (double)npart) / vol; //Pressure
    glob_av[ipr] += stima_press;
    glob_av2[ipr] += stima_press*stima_press;
    err_press=Error(glob_av[ipr],glob_av2[ipr],iblk);

//g(r)
    for(int i=igofr;i<igofr+nbins;i++){
      r=bin_size*(i-igofr);
      stima_g[i]=blk_av[i]/(blk_norm*rho*npart*4.0*pi*(pow(r+bin_size,3)-pow(r,3))/3.0) ; 
      glob_av[i] += stima_g[i];
      glob_av2[i] += stima_g[i]*stima_g[i];
      err_g[i]=Error(glob_av[i],glob_av2[i],iblk);
      Gofr<<iblk<<" "<<i<<" "<<" "<<stima_g[i]<<endl;
...
//last g(r)
  ...
  if(iblk==nblk){
      for(int i=igofr;i<igofr+nbins;i++){
        r=bin_size*(i-igofr);
        Gave<<r<<" "<<stima_g[i]<<' '<<glob_av[i]/(double)iblk << " " << err_g[i] << endl;
        }
  }
...
//Pressure
    Press << iblk << " " << stima_press <<" " << glob_av[ipr]/(double)iblk <<" " << err_press << endl;
...
void ConfFinal(void)
{
  ...
  WriteConf.open(folder+"config.out");
  WriteVelocity.open(folder+"velocity.out");
  for (int i=0; i<npart; ++i)
  {
